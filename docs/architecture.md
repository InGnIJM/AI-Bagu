...
本文面向维护者，说明当前源码的共享核心、会话、评分、存储和平台安全边界。当前公开 beta.5（code 5）使用 SQLite v3，并已包含题包、面经专题和备份 v3；历史 beta.4（`ac53f34`）使用 SQLite/备份 v2，不包含这些能力。使用方法见[用户指南](user-guide.md)，请求字段见 [HTTP API](api.md)，源码与产物验收分开记录在[验证记录](validation.md)。
