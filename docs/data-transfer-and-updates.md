...
beta.4 把再次校验通过的 APK 写入系统 `PackageInstaller.Session`：使用完整安装模式，写入后执行 `fsync`，再用显式、可变且只指向应用内非导出 Activity 的回调提交。Android 15/API 35+ 创建回调 `PendingIntent` 时显式授予 creator BAL，只用于启动系统返回的安装确认界面；确认回调到达、启动成功或失败分别写入固定 `confirm` 阶段，不记录 Intent、路径、Session ID 或系统错误正文。不再把 `content://` APK URI 交给通用文件打开流程，也不指定厂商安装器包名。最终完整签名验证仍由 Android 安装器执行，应用解析到的证书信息本身不等于完整 APK 密码学验签。该修复已进入 beta.4 签名 APK；报告问题的 API36 物理手机及厂商系统仍须继续验收。
