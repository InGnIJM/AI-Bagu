...
这里按实际操作介绍练习、模型、题库和备份。首次启动请先看[快速开始](../README.md#快速开始)。两种导出、确认导入、诊断日志、格式恢复和应用内更新已包含在公开 beta.4；本页标注为「当前源码」的面经题包、SQLite/备份 v3 尚未随 beta.4 发布。下载入口与源码范围见[版本与文档范围](README.md#版本与文档范围)。

## 练习与复习

### 选择模式

答题方式仍分为「答题模式」和「背题模式」；当前源码的练习入口另分为「日常复习」与「面经模拟」。公开 beta.4 只有原来的日常抽题界面，不应按下面的题包步骤操作。
...
- 面经技术题：题型为 `review`，完全沿用上述答题、背题、模型评卷、自评和间隔调度；题包答案显示「题包参考答案 · 已复核」，不冒充原帖标准答案。
...
当前源码中的日常统计与日常抽题使用同一资格范围：关闭某题包的「加入日常复习」后，该包题不再计入日常总量、分类掌握度或待抽题数，但专题仍保留、模拟仍可启动、已经形成的进度也不会清除。不同面经即使出现相同文字，仍按各自 `pack_id + stable_question_id` 独立计分，不做自动语义合并。
...
当前源码的题目列表会额外显示题包名、稳定题号、`review|prepare` 类型、审校状态和来源。题包题由对应 revision 管理，在普通题目入口只能查看，不能修改或删除；CSV 和网页新增仍只创建本地 `review` 题，不承载专题、章节或题包稳定 ID。

## 面经模拟与题包

本节只适用于 2026-08-30 的当前开发源码，尚未进入公开 beta.4。仓库没有内置 `.bagu-pack`、首包下载地址或在线商店，也没有读取或迁移当前面经源目录。

### 安装或升级本地题包

1. 先结束当前日常/面经练习；有 open 会话时可以预览，但安装或升级会返回冲突，不会自动结束本轮。
2. 在「设置 → 面经题包」选择本地 `.bagu-pack`。程序先完整校验固定成员、内容哈希、稳定 ID、来源与专题引用，并展示题包名、显示版本、revision、题数、专题数和当前安装状态。
3. 只有核对预览后点击明确确认，才安装刚才检查的同一份内存字节。取消、校验失败或版本冲突都会清除待安装内容。
4. 安装后回到「面经模拟」选择专题；也可在题包列表关闭或打开「加入日常复习」。
...
「导出纯题库」保存本地题内容；当前源码的 v3 还保存题包元数据、题包题与来源、专题/章节顺序和「加入日常复习」偏好。「导出含进度备份」另外保存可复习题的等级、次数和复习日期。两者均**不包含模型配置、API Key、草稿、会话、submission 或评分分析/历史答案快照**，也不下载答案中的图片。换设备迁移进度时应选择后者。
...
当前源码导出 `.bagu-backup` schema v3，严格包含 `manifest.json`、`questions.json`、`packs.json`、`experiences.json`；仍能读取 v1/v2，其中 v1 按历史 progress 语义处理。公开 beta.4 仍只导出/读取两成员 v1/v2，不能读取 v3；数据库版本与备份格式版本也不要混为一谈。预览与恢复使用同一份已校验字节，取消确认不执行恢复。若结果未知，先核对数据，不要直接重复导入。启动程序及 HTTP 接口仍可能初始化／迁移数据库，不能把 HTTP 文件预览当作数据库升级的只读演练。详细规则见[数据迁移](data-transfer-and-updates.md#电脑与手机之间迁移)，程序接入见[备份 API](api.md#备份与恢复)。
...
SQLite 数据库版本与 `.bagu-backup` 文件格式版本不是一回事。当前源码使用 SQLite v3 并可事务迁移 v2；公开 beta.4 使用 SQLite v2。升级会保留旧题 ID、进度、会话和评分结果，并为题包/专题、有序会话和完成类型增加结构；升级后的 v3 库不能直接交给旧程序使用。
...
公开 beta.4 已包含此功能；旧包没有入口时需要先更新，未记录的历史错误无法追补。
...
截至 2026-08-29，[Beta 清单](https://ingnijm.github.io/AI-Bagu/updates/beta.json) 已提供 beta.4，[Stable 清单](https://ingnijm.github.io/AI-Bagu/updates/stable.json) 为有效空通道。之前的双通道 404 不再是本次上线后的正常状态；若你的手机仍失败，先手动重试并保留新的时间与编号，不能根据维护者电脑可访问就认定手机网络也正常。
