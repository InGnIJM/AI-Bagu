...
这里按实际操作介绍练习、模型、题库和备份。首次启动请先看[快速开始](../README.md#快速开始)。功能说明以已核对的基础版本为准，开发中的迁移与更新功能见[版本与文档范围](README.md#版本与文档范围)。
...
Android 在「设置」中导出 `.bagu-backup`，使用系统选择器保存到应用以外的位置。恢复前先结束当前练习、备份现有数据，再选择要恢复的文件。

基础版本的 `.bagu-backup` 保存题目、答案、来源 URL 和复习进度，**不包含模型配置、API Key、草稿、会话或评分分析**。电脑已有对应[备份 API](api.md)，基础版本的桌面页面没有备份按钮。正在开发的新导出模式及导入预览，不包含在本节的基础版本说明内。

恢复按分类和题干合并：同名题的答案、URL 与进度会被备份覆盖，新题新增，备份外的题目不删除；已有会话及评分分析历史不改。导出允许有进行中的练习，但不包含该会话；恢复则要求先结束练习。
