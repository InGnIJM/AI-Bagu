...
答题方式仍分为「答题模式」和「背题模式」；beta.5 的练习入口另分为「日常复习」与「面经模拟」。使用 beta.4 或更早版本时没有题包入口，需要先更新。

- **日常复习**：选择「全部分类」或一个 MySQL、网络、Agent 等知识分类，再点击「抽 5 题开始」。程序优先抽到期复习题，再随机补新题；只纳入本地普通题，以及已开启「加入日常复习」的题包中未退役 `review` 题。`prepare` 题不会进入这里。
- **面经模拟**：先按方向、公司和岗位筛选专题，再选整套或具体章节；有多个章节时默认选题包标记的推荐章节。专题按题包保存的章节顺序与题目顺序进行，刷新或重启服务后也不改用数据库 ID 排序。题包即使关闭了日常复习，仍可在这里模拟。

两种入口共用同一把会话锁：每份数据库最多一轮进行中的日常或面经会话。开始另一轮前应继续当前会话或主动 `skip`。可用题不足时不会凭空补题；面经整套或章节没有未退役题时会拒绝启动。

- 答题模式：先输入回答，再点击「发给模型评判」。桌面也支持 Ctrl + Enter 提交。
- 背题模式：直接展示题库答案，阅读后用「不会 / 勉强 / 会了 / 秒答」自评，对应 again / hard / good / easy；支持数字键 1～4。
- 面经技术题：题型为 `review`，完全沿用上述答题、背题、模型评卷、自评和间隔调度；题包答案由 AI 生成并经维护者复核，显示「题包参考答案 · 已复核」，不是原帖作者或面试公司的标准答案。
- 面经准备项：题型为 `prepare`，只展示人工审校的准备提示和「已准备 / 跳过此准备项」。它不显示作答框、不调用模型、不增加 `times_seen`，也不修改等级或到期日。
...
首页显示总题量、今日复习、未学习、已掌握和本轮进度。“今日复习”不包含从未学习的新题；分类掌握度按各类 `level >= 3` 的题数计算，Android 的入口在「概览」。评分成功后看板会立即刷新；再次遇到某题的时间由评级和当前等级决定，不是所有 easy 都固定 7 天。精确间隔与「已掌握」的计算规则见[调度规则](architecture.md)。

beta.5 的日常统计与日常抽题使用同一资格范围：关闭某题包的「加入日常复习」后，该包题不再计入日常总量、分类掌握度或待抽题数，但专题仍保留、模拟仍可启动、已经形成的进度也不会清除。不同面经即使出现相同文字，仍按各自 `pack_id + stable_question_id` 独立计分，不做自动语义合并。
...
桌面点击「题库管理」，Android 点击「题库」。支持按题目、答案和分类搜索，或选择分类筛选。点击「查看答案」展开正文。
...
beta.5 的题目列表会额外显示题包名、稳定题号、`review|prepare` 类型、审校状态和来源。题包题由对应 revision 管理，在普通题目入口只能查看，不能修改或删除；CSV 和网页新增仍只创建本地 `review` 题，不承载专题、章节或题包稳定 ID。
...
1. 从 [beta.5 Release](https://github.com/InGnIJM/AI-Bagu/releases/tag/v0.1.0-beta.5) 单独下载题包附件；安装 APK 不会自动取得题包。
2. 先结束当前日常/面经练习；有 open 会话时可以预览，但安装或升级会返回冲突，不会自动结束本轮。
3. 在「设置 → 面经题包」选择本地 `.bagu-pack`。程序先完整校验固定成员、内容哈希、稳定 ID、来源与专题引用，并展示题包名、显示版本、revision、题数、专题数和当前安装状态。
4. 只有核对预览后点击明确确认，才安装刚才检查的同一份内存字节。取消、校验失败或版本冲突都会清除待安装内容。
5. 安装后回到「面经模拟」选择专题；也可在题包列表关闭或打开「加入日常复习」。

首次安装默认加入日常复习。更高 revision 按稳定 ID 更新正文、答案、准备提示、来源和专题结构，同时保留题目整数主键、学习进度、历史评分答案快照和本地开关。相同 revision、相同内容重复导入是幂等成功；相同 revision 但内容不同或 revision 降级会整包拒绝。新版遗漏的旧 ID 不会被当作删除，只有题包显式标为 `retired` 的题才停止进入新日常抽题和专题会话。题型发生实质变化必须由制作者分配新稳定 ID。

桌面浏览器会把文件读取一次，在内存中以同一份 base64 完成预览和确认，不持久化待安装字节。Android 使用系统文件选择器，由原生层持有同一份受限字节；allowlist 预览只留在原生确认框，JS/WebView 不接收预览或 raw/body/archive/base64/hash、题目、答案、准备提示，完成后只收到 allowlist 过滤、脱敏的结果状态/字段。Activity 配置重建可重新显示仍待确认的原生状态，但不会自动安装，进程死亡后必须重新选择文件。旧 Android 宿主没有对应桥接时只提示更新，不会退回到让网页读取文件正文。
...
仓库不保存可安装的真实题包。beta.5 的 27 专题／748 题正式 r1 由仓库外私有流程完成冻结和逐题审校，字节仅作为 GitHub Release 的独立 `.bagu-pack` 附件公开，不提交 Git。题目答案由 AI 生成，维护者已复核并接受为正式参考答案，但不表示原帖作者或面试公司的标准答案。仓库、public/internal 种子与公开 APK 都排除 `.bagu-pack` 和私有 catalog；首版不支持在线商店、应用内自动题包下载／更新、自动语义去重或物理卸载。

**以下格式修复自公开 beta.4 起已包含：**
...
「导出纯题库」保存本地题内容；beta.5 的 v3 还保存题包元数据、题包题与来源、专题/章节顺序和「加入日常复习」偏好。「导出含进度备份」另外保存可复习题的等级、次数和复习日期。两者均**不包含模型配置、API Key、草稿、会话、submission 或评分分析/历史答案快照**，也不下载答案中的图片。换设备迁移进度时应选择后者。

恢复本地题仍按分类和题干合并；题包题按 `pack_id + stable_question_id` 合并，专题按稳定 ID 恢复。两种模式都会恢复内容、题包结构与开关；纯题库保留目标库已有进度，新题从零开始，含进度备份覆盖调度但不重新计算日期。`prepare` 项始终保持零调度。目标库独有的题和历史记录不删除；已有会话及评分分析历史不改。题包降级、同 revision 内容冲突或题型冲突会使整批恢复失败。导出允许有进行中的练习，但不包含该会话；恢复则要求先结束练习。

beta.5 导出 `.bagu-backup` schema v3，严格包含 `manifest.json`、`questions.json`、`packs.json`、`experiences.json`；仍能读取 v1/v2，其中 v1 按历史 progress 语义处理。beta.4 仍只导出/读取两成员 v1/v2，不能读取 v3；数据库版本与备份格式版本也不要混为一谈。预览与恢复使用同一份已校验字节，取消确认不执行恢复。若结果未知，先核对数据，不要直接重复导入。启动程序及 HTTP 接口仍可能初始化／迁移数据库，不能把 HTTP 文件预览当作数据库升级的只读演练。详细规则见[数据迁移](data-transfer-and-updates.md#电脑与手机之间迁移)，程序接入见[备份 API](api.md#备份与恢复)。

导出与恢复均限制为 10000 题、ZIP 压缩文件 20 MiB、JSON 解压后合计 50 MiB。损坏、字段无效、超限或校验失败时整批拒绝，不部分写入或截断题库。文件选择器可能留下零字节占位文件，它不是有效备份；请确认导出提示成功并检查文件。
...
SQLite 数据库版本与 `.bagu-backup` 文件格式版本不是一回事。beta.5 使用 SQLite v3 并可事务迁移 v2；beta.4 使用 SQLite v2。升级会保留旧题 ID、进度、会话和评分结果，并为题包/专题、有序会话和完成类型增加结构；升级后的 v3 库不能直接交给旧程序使用。
...
公开 beta.4 及之后版本已包含此功能；旧包没有入口时需要先更新，未记录的历史错误无法追补。
...
截至 2026-08-31，[Beta 清单](https://ingnijm.github.io/AI-Bagu/updates/beta.json) 已提供 beta.5，[Stable 清单](https://ingnijm.github.io/AI-Bagu/updates/stable.json) 仍为有效空通道。更新清单只描述 APK，不会自动下载题包；若你的手机检查失败，先手动重试并保留新的时间与编号，不能根据维护者电脑可访问就认定手机网络也正常。
