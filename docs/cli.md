...
查看参数帮助：`python bagu.py --help` 或 `python bagu.py draw --help`。不要把表中的尖括号占位符原样输入。CLI 没有模型配置、CSV/题包导入、专题启动或备份恢复子命令；这些操作使用网页、Android 或 [HTTP API](api.md)。
...
- 抽题只考虑符合日常资格的新题（`next_due` 为空）和到期题：本地普通 `review` 题始终可参与；题包题还必须未退役且对应题包已开启「加入日常复习」；`prepare` 永不进入 CLI draw。先取到期复习题，再随机补新题。可选题不足时实际数量会少于请求数；没有可选题时不创建会话。
- `stats` 与 draw 使用同一资格范围。“今日复习”只统计已经进入调度且到期的题；“未学习”指 `next_due` 为空的新题，“可抽题”是两者之和。“已刷”指至少复习过一次；`level >= 3` 计入“已掌握”。关闭题包后，其题不出现在日常总量/分类统计中，但题包和进度仍保留。
- 面经模拟仅通过共享网页/HTTP 启动。它与 CLI 共用唯一 open 会话锁，因此网页已经开始面经时，CLI `draw` 仍会按既有协议拒绝并列出未完成题目；CLI `grade` 只适用于其中的 `review` 项，`prepare` 由专用 HTTP 完成接口处理。
...
格式命令只接受已经存在、且 schema 为当前版本（当前源码为 SQLite v3）的数据库，不执行初始化或升级；旧版或高版本数据库会被拒绝。`--dry-run` 使用 SQLite 只读连接；需要升级的数据库须另行完整备份并升级，不能用格式修复代替数据库迁移。该修复只处理本地题，不改题包所有权或专题结构。
...
当前源码引入题包后，Hermes 协议仍是 `grade <session_id> <题id> <again|hard|good|easy>`，没有新增参数或自动面经选择。Hermes 按 `draw` 返回的日常 `review` 题工作，不应尝试用 `grade` 完成 `prepare` 项。
