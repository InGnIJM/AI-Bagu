...
> 文档定位：Android 初始确认方案与决策记录，移动隔离、稳定签名、种子和备份等约束仍有效。下文“实施中”、当时授权和未来计划保留历史语境，不表示当前待办或新的操作授权。现行结构见[架构与数据约束](../../architecture.md)，构建操作见 [Android Beta](../../android-beta.md)，具体源码/APK 完成范围以[验证记录](../../validation.md)为准。
