...
> 文档定位：基础设计与历史演进记录。会话锁、CLI 和失败不评分等约束仍有效；单模型写接口、Hermes 导入、easy 无答案和紫色 UI 已被后续实现替代。现行行为见[架构与数据约束](../../architecture.md)及 [HTTP API](../../api.md)，使用说明见[用户指南](../../user-guide.md)。下文保留当时正文，不据此回退接口或界面。
...
>
> 2026-08-28 AI 评卷更新：下文“easy 不含答案”及“仅非 easy 要求完整答案”的历史约定已被取代。AI 四档评分均保存答案及来源，优先题库、缺失时模型补充；easy 仅在页面默认折叠。提示词采用明确标准、两题八条示例及具体反馈，解析必须包含按顺序且唯一的 GRADE / COMMENT / ANSWER，点评非空。当前细节见 README 的“AI 评分与学习反馈”和故障恢复设计的 v2 补充。CLI/Hermes 和自评规则不变。
