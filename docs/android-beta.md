...
备份、题包、CSV、模板、诊断和更新检查/下载/安装使用一个进程级原生操作仲裁器，精确 token 和 operation ID 防止迟到 picker/callback 释放另一项操作。真实 SAF provider、旋转、系统杀进程、WebView 事件、返回键和安装器并发目前只有自动化/编译证据，尚未在设备或模拟器运行本次题包仪器测试。
...
新迁移／更新验收还须覆盖 API 29／36 上的备份/题包同字节确认与取消、Activity 重建、进程死亡、真实 SAF provider、返回键、文件/更新互斥、权限、缓存损坏、安装取消和两版本覆盖升级；只能使用隔离的模拟数据，不能触碰个人应用数据。本次题包相关 AndroidTest 只完成编译，未在设备运行。
