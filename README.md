...
当前开发源码还增加了独立的「面经模拟」和本地 `.bagu-pack` 题包：`category` 继续表示 MySQL、网络等知识分类，面经则按方向、公司、岗位和章节选择，不会被塞进普通分类。**这项能力尚未进入下方公开 beta.4 APK，也没有随仓库提供真实首包。**
...
- **按专题演练面经（当前开发源码）**：从本地明确导入的题包选择方向、公司、岗位、整套或章节，并按题包顺序恢复进度。
...
初次打开没有题目时，可从空题库页导入另一端的 `.bagu-backup`，也可以进入「题库管理」新增题目或导入 CSV。使用当前开发源码时，还可在设置中选择本地 `.bagu-pack`，先核对完整校验后的预览，再明确确认安装；仓库本身不附带题包。想从项目已配置的公开来源抓题，也可以另开终端执行：
...
APK 放在 GitHub Releases，不提交到源码目录。公开包首次安装为空题库，可导入自己的 CSV 或 `.bagu-backup`；不要把截图中的 408 题理解为默认附带。已发布 beta.4 不含面经题包功能或任何 `.bagu-pack`；当前源码的新 Android 导入能力仍需以后重新构建、验收并发布才会出现在手机安装包中。**更新不要先卸载**，同包名、同签名覆盖升级保留原有本地数据。安装与构建方式见 [Android 指南](docs/android-beta.md)，本版的精确产物与验证范围见[beta.4 验收记录](docs/validation.md#beta4-公开发布)。
...
1. **选练习入口**：公开 beta.4 选择「答题模式」或「背题模式」后抽题。当前开发源码进一步分为「日常复习」和「面经模拟」：日常复习仍可按知识分类抽 5 题；面经模拟按方向、公司、岗位选择专题，再选择整套或推荐章节。
...
打开「题库管理」（Android 为「题库」），可以搜索题目或答案、筛选分类、展开答案、新增和修改本地题目。当前开发源码会把题包题标明来源、稳定 ID 和类型，并保持只读；题包升级由稳定 ID 修订内容，不允许在通用编辑/删除入口绕过所有权。
...
先导出**含进度备份**，并确认已保存到应用外；纯题库导出不带复习进度。当前开发源码导出的 `.bagu-backup` v3 还保存已安装题包快照、专题结构、日常复习开关及对应稳定 ID 进度；公开 beta.4 导出的是旧 v2，不具备这些字段。两者均不包含模型配置、API Key、草稿、会话或评分分析；卸载会删除应用私有数据。详见[迁移范围与恢复](docs/data-transfer-and-updates.md#电脑与手机之间迁移)。
